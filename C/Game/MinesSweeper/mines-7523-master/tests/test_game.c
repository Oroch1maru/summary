...
TEST marked_mine_for_wining() {
    Game *game = create_game();
    Board *board = create_board(3, 3, 9);
    game->board = board;
    for (int row = 0; row < game->board->row_count; row++) {
        for (int column = 0; column < game->board->column_count; column++) {
            if(game->board->tiles[row][column]->is_mine ){
                mark_tile(game, row, column);
                
            }
        }
    }
    ASSERT_EQ(SOLVED, game->game_state);
    destroy_game(game);
    PASS();
}
...
TEST marked_tile_with_bad_coordinates() {
    Game *game = create_game();
    Board *board = create_board(10, 10, 5);
    game->board = board;
    mark_tile(game, -1, -1);
    mark_tile(game, game->board->row_count, game->board->column_count);
...
SUITE (test_game) {
    RUN_TEST(marked_mine_for_wining);
    RUN_TEST(marked_tile_with_bad_coordinates);
    RUN_TEST(open_mine);
